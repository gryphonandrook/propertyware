  ...
  class UpdateLeaseJournalEntry
    ...
    def self.openapi_types
      {
        :'comments' => :'String',
        :'date' => :'Date',
        :'journal_entry_split' => :'Array<JournalEntrySplit>',
        :'lease_id' => :'Integer',
        :'ref_no' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::UpdateLeaseJournalEntry` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::UpdateLeaseJournalEntry`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [comments, date, journal_entry_split, lease_id, ref_no].hash
    end
