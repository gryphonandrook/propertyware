...
module Propertyware
  class LateFeeRule
    ...
    # Minimum amount due to trigger the late fee process.
    attr_accessor :minimum_due

    # Textual description of the late fee calculation.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::LateFeeRule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::LateFeeRule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'minimum_due')
        self.minimum_due = attributes[:'minimum_due']
      end
    ...
    def hash
      [due_day, fee_type, id, minimum_due, summary].hash
    end
