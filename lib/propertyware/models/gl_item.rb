...
module Propertyware
  ...
  class GLItem
    ...
    # Id of the owner associated with this transaction.
    ...
    # Transaction date.
    attr_accessor :tx_date

    # Id of the vendor associated with this transaction.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::GLItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::GLItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'glentries')
        if (value = attributes[:'glentries']).is_a?(Array)
          self.glentries = value
        end
      end
      ...
      if attributes.key?(:'tx_date')
        self.tx_date = attributes[:'tx_date']
      end
    ...
    def hash
      [comment, created_by, created_date_time, glentries, id, last_modified_by, last_modified_date_time, lease_id, owner_id, transaction_type, tx_date, vendor_id].hash
    end
