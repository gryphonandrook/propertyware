...
module Propertyware
  ...
  class Conversation
    ...
    # Id of the entity related to this conversation.
    attr_accessor :entity_id

    # Type of the entity related to this conversation.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::Conversation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::Conversation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ["MANAGEMENT_TEAM", "OWNER", "TENANT"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["MANAGEMENT_TEAM", "OWNER", "TENANT"])
    ...
    def hash
      [comments, created_by, created_date_time, entity_id, entity_type, id, last_modified_by, last_modified_date_time, subject, type].hash
    end
