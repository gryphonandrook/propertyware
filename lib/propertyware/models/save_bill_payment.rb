  ...
  class SaveBillPayment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::SaveBillPayment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::SaveBillPayment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      payment_method_validator = EnumAttributeValidator.new('String', ["Check", "Cash", "Bank Bill Pay", "Online Bank Transfer"])
    ...
    def payment_method=(payment_method)
      validator = EnumAttributeValidator.new('String', ["Check", "Cash", "Bank Bill Pay", "Online Bank Transfer"])
    ...
    def hash
      [bills, check_number, memo, payment_account_id, payment_date, payment_method, vendor_id].hash
    end
