...
module Propertyware
  ...
  class Equity
    ...
    # Indicates if Transfer balance at year end to Retained earnings.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::Equity` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::Equity`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [discount_account, escrow_account, exclude_from1099, exclude_from_pay_in_full, exclude_late_fee, rent_account, tax_account, transfer_balance_to_retained_earnings].hash
    end
