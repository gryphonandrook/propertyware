...
module Propertyware
  class JournalEntrySplitResponse
    # Credit amount.
    ...
    # Id of the general ledger account associated with this journal entry split.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::JournalEntrySplitResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::JournalEntrySplitResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @debit_amount.nil?
        invalid_properties.push('invalid value for "debit_amount", debit_amount cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @debit_amount.nil?
    ...
    def hash
      [credit_amount, debit_amount, gl_account_id, id].hash
    end
