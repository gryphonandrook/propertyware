...
module Propertyware
  ...
  class Inspection
    ...
    # Date and time the inspection occurred (customer time zone).
    attr_accessor :inspected_date_and_time
    ...
    # Id of the portfolio associated with this inspection.
    attr_accessor :portfolio_id

    # Date and time the inspection is/was scheduled to occur (customer time zone).
    attr_accessor :scheduled_date_and_time
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::Inspection` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::Inspection`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'inspected_date_and_time')
        self.inspected_date_and_time = attributes[:'inspected_date_and_time']
      end

      if attributes.key?(:'inspection_areas')
        if (value = attributes[:'inspection_areas']).is_a?(Array)
          self.inspection_areas = value
        end
      end
      ...
      if attributes.key?(:'scheduled_date_and_time')
        self.scheduled_date_and_time = attributes[:'scheduled_date_and_time']
      end
