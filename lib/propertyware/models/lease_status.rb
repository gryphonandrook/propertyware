...
module Propertyware
  # Lease Status
  class LeaseStatus
    attr_accessor :define_active

    attr_accessor :id

    attr_accessor :order_index

    attr_accessor :portal_login_enabled
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::LeaseStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::LeaseStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'define_active')
        self.define_active = attributes[:'define_active']
      end
      ...
      if attributes.key?(:'portal_login_enabled')
        self.portal_login_enabled = attributes[:'portal_login_enabled']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          define_active == o.define_active &&
          id == o.id &&
          order_index == o.order_index &&
          portal_login_enabled == o.portal_login_enabled &&
          readonly == o.readonly &&
          status == o.status
    end
    ...
    def hash
      [define_active, id, order_index, portal_login_enabled, readonly, status].hash
    end
