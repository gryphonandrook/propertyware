  ...
  class SaveProspectContact
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::SaveProspectContact` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::SaveProspectContact`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'percentageship')
        self.percentageship = attributes[:'percentageship']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          percentageship == o.percentageship &&
    ...
    def hash
      [address, email, first_name, home_phone, id, last_name, mobile_phone, other_phone, percentageship, work_phone].hash
    end
