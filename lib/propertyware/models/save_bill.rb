...
module Propertyware
  ...
  class SaveBill
    ...
    # Indicates if it is a owner draw.
    ...
    def self.openapi_types
      {
        :'bill_date' => :'Date',
        :'bill_number' => :'Integer',
        :'bill_splits' => :'Array<SaveBillSplit>',
        :'comments' => :'String',
        :'due_date' => :'Date',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::SaveBill` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::SaveBill`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'owner_draw')
        self.owner_draw = attributes[:'owner_draw']
      end
    ...
    def list_invalid_properties
      ...
      if @bill_date.nil?
        invalid_properties.push('invalid value for "bill_date", bill_date cannot be nil.')
      end
    ...
    def hash
      [bill_date, bill_number, bill_splits, comments, due_date, markup_account_id, markup_percentage, owner_draw, payment_date, ref_no, terms, vendor_id, work_order_id].hash
    end
