...
module Propertyware
  class AutoJournalEntry
    ...
    # Auto journal entry name.
    attr_accessor :name

    # Next post date.
    attr_accessor :next_post_date
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::AutoJournalEntry` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::AutoJournalEntry`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'last_post_date')
        self.last_post_date = attributes[:'last_post_date']
      end
      ...
      if attributes.key?(:'next_post_date')
        self.next_post_date = attributes[:'next_post_date']
      end

      if attributes.key?(:'pay_day')
        self.pay_day = attributes[:'pay_day']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          last_post_date == o.last_post_date &&
          name == o.name &&
          next_post_date == o.next_post_date &&
          pay_day == o.pay_day &&
    ...
    def hash
      [comments, created_by, created_date_time, credit, debit, end_date, frequency, id, last_modified_by, last_modified_date_time, last_post_date, name, next_post_date, pay_day, start_date].hash
    end
