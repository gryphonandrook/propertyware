...
module Propertyware
  ...
  class CheckSplit
    ...
    # Id of the building associated with this check split.
    attr_accessor :building_id

    # Comments.
    attr_accessor :comments

    # Id of the general ledger account associated with this check split.
    attr_accessor :gl_account_id

    # Unique identifier.
    attr_accessor :id

    # Id of the portfolio associated with this check split.
    attr_accessor :portfolio_id

    # Id of the unit associated with this check split.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::CheckSplit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::CheckSplit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [amount, building_id, comments, gl_account_id, id, portfolio_id, unit_id].hash
    end
