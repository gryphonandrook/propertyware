...
module Propertyware
  ...
  class SaveBillSplit
    ...
    # Indicates if markup discount split.
    attr_accessor :markup_discount_split

    # Indicates if Bill Split is paid.
    attr_accessor :paid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::SaveBillSplit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::SaveBillSplit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [amount, amount_paid, building_id, comments, gl_account_id, id, markup_discount_split, paid, portfolio_id, unit_id].hash
    end
