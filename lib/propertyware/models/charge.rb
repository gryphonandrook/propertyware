...
module Propertyware
  class Charge
    ...
    # Description of the day when the auto charge is due.
    attr_accessor :due_day

    # End date of auto charge.
    attr_accessor :end_date

    # Frequency of the auto charge.
    attr_accessor :frequency

    # ID of the General Ledger Account associated to this auto charge.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::Charge` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::Charge`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account, amount, description, due_day, end_date, frequency, gl_account_id, id, last_post_date, reference_number, start_date].hash
    end
