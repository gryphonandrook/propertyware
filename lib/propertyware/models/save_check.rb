...
module Propertyware
  ...
  class SaveCheck
    ...
    def self.openapi_types
      {
        :'check_date' => :'Date',
        :'check_splits' => :'Array<SaveCheckSplit>',
        :'comments' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Propertyware::SaveCheck` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Propertyware::SaveCheck`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [check_date, check_splits, comments, destination_account_id, ref_no, to_be_printed, vendor_id].hash
    end
